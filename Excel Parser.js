...
if (attachGR.next()) {
  ...
  while (parser.next()) {
    ...
    if (cutTaskForChange(row[numberHeader])) {
      successArr.push(row[numberHeader]);
    } else {
      failedArr.push(row[numberHeader]);
    }
  }
  current.work_notes =
...
function cutTaskForChange(chgNumber) {
  var trimmedNumber = chgNumber.trim();
  var chg = new GlideRecord("change_request");
  if (chg.get("number", trimmedNumber)) {
    // Create change task
    var changeTask = new GlideRecord("change_task");
    changeTask.initialize();
    changeTask.create_from =
      "Generated by Workflow Script: " + current.getValue("number");
    changeTask.change_request = chg.getUniqueValue();
    changeTask.u_validator = chg.getValue("requested_by");
    changeTask.assignment_group = assignGroup;
    changeTask.assigned_to = assignTo;
    if (prefix != "")
      changeTask.short_description =
        environmentName + " (" + prefix + ") - " + chg.short_description;
    else
      changeTask.short_description =
        environmentName + " - " + chg.short_description;
    changeTask.description =
      "Please complete the " + environmentName + " environment";
    ...
    workflow.info(
      "Inserted Task: " + changeTask.getValue("number") + " for " + chg.number
    );
    return true;
  } else {
    workflow.error("Skipped Change: " + trimmedNumber);
    return false;
  }
}
